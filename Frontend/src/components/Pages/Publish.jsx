    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="container mx-auto px-6 pt-24 pb-16">
        ...
        <div className="mb-6 text-center">
          <button onClick={() => setShowPublishModal(true)} className="bg-green-500 text-white px-6 py-2 rounded-lg hover:bg-green-600 transition-all">
            Publish a Course
          </button>
        </div>
          ...
          <Modal title="Unpublished Courses" onClose={() => setShowPublishModal(false)}>
            {unpublishedCourses.length === 0 ? (
              ...
              unpublishedCourses.map((course) => (
                <CourseCard key={course._id} course={course} expandedCourse={expandedCourse} setExpandedCourse={setExpandedCourse}>
                  <button onClick={() => handlePublishCourse(course)} className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition-all mt-2">
                    Publish
                  </button>
                </CourseCard>
              ))
...
const CourseSection = ({ title, courses, expandedCourse, setExpandedCourse, handleDeleteCourse }) => (
  <div className="mb-8">
    ...
    {courses.length === 0 ? (
      ...
      courses.map((course) => (
        <CourseCard key={course._id} course={course} expandedCourse={expandedCourse} setExpandedCourse={setExpandedCourse} handleDeleteCourse={handleDeleteCourse} />
      ))
    )}
  </div>
);
...
const CourseCard = ({ course, expandedCourse, setExpandedCourse, handleDeleteCourse, children }) => (
  <div className="bg-white p-6 rounded-lg shadow-lg mb-4">
    ...
    <button onClick={() => setExpandedCourse(expandedCourse === course._id ? null : course._id)} className="text-blue-600 hover:underline">
    ...
    {expandedCourse === course._id && (
      <div className="mt-4">
        ...
        <p><strong>Topics:</strong> {course.topic}</p>
        <h5 className="font-bold mt-2">Chapters:</h5>
        {course.chapters.map((chapter, index) => (
          <p key={chapter._id} className="text-gray-700">• {chapter.content}</p>
        ))}
      </div>
    )}
    {children}
...
const Modal = ({ title, children, onClose }) => (
  <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
    <div className="bg-white p-6 rounded-lg shadow-lg w-1/3 max-h-[80vh] overflow-y-auto">
      <h3 className="text-2xl font-semibold text-gray-900 mb-4">{title}</h3>
      {children}
      <button onClick={onClose} className="mt-4 bg-red-500 text-white px-6 py-2 rounded-lg hover:bg-red-600 transition-all">Close</button>
    </div>
  </div>
);
